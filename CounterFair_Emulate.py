...
parser.add_argument('-do_l2', type=str2bool, nargs='?', const=True, default=True,
                    help="Perform L2 Train/Test (Warning: TAKES TIME)")
parser.add_argument('-save_l2', type=str2bool, nargs='?', const=True, default=False,
                    help="Save L2 Train/Test Models (Warning: LARGE FILES)")
...
def get_data_preprocess():
    ...
    sense_cols = ['Amerindian', 'Asian', 'Black', 'Hispanic', 'Mexican', 'Other', 'Puertorican', 'White', 'male',
                  'female']
...
def Unfair_Model_Replication(law_train_dic, law_test_dic):
    ...
    lr_unfair.fit(np.hstack((law_train_dic['a'], np.array(law_train_dic['ugpa']).reshape(-1, 1),
                             np.array(law_train_dic['lsat']).reshape(-1, 1))), law_train_dic['zfya'])

    preds = lr_unfair.predict(np.hstack((law_test_dic['a'], np.array(law_test_dic['ugpa']).reshape(-1, 1),
...
def FTU_Model_Replication(law_train_dic, law_test_dic):
    ...
    lr_unaware.fit(
        np.hstack((np.array(law_train_dic['ugpa']).reshape(-1, 1), np.array(law_train_dic['lsat']).reshape(-1, 1))),
        law_train_dic['zfya']);

    preds = lr_unaware.predict(
        np.hstack((np.array(law_test_dic['ugpa']).reshape(-1, 1), np.array(law_test_dic['lsat']).reshape(-1, 1))))
