...
model {
  ...
  // Level 3:
  ugpa0      ~ normal(0, 1); // all seem to have non-informative priors!
  eta_u_ugpa ~ normal(0, 1);
  lsat0      ~ normal(0, 1);
  eta_u_lsat ~ normal(0, 1);
  eta_u_zfya ~ normal(0, 1);

  eta_a_ugpa ~ normal(zero_K, one_K); // letting the weights come from a joint normal
  ...
  ugpa ~ normal(ugpa0 + eta_u_ugpa * u + a * eta_a_ugpa, sigma_g); // why the intercept when you haven't dropped fem?
